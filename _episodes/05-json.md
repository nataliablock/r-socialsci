...
the lesson, let's coerce it to be a tibble and use `glimpse` to take a peek inside:
...
~~~
Error in json_data %>% as_tibble(): could not find function "%>%"
~~~
{: .error}
...
~~~
Error in glimpse(json_data): could not find function "glimpse"
~~~
{: .error}
...
~~~
Error in write_csv(flattened_json_data, file = "data_output/SAFI_from_JSON.csv"): could not find function "write_csv"
~~~
{: .error}
...
~~~
Error in write_csv(json_data$F_liv[[1]], file = "data_output/F_liv_row1.csv"): could not find function "write_csv"
~~~
{: .error}
